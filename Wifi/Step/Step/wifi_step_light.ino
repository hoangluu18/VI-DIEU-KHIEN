...
Stepper myStepper = Stepper(2048, D3, D1, D2, D0); //  INT1 INT2 INT3 INT4
...
const char index_html[] PROGMEM = R"rawliteral(
  <html>
  <head>
    <title>STEPPER MOTOR AND LIGHT</title>
  </head>
  <body>
    <div>
      <p style="font-size: 25px"><b>Cam bien anh sang:</b> <span id="sensor_value"></span></p>
    </div>
...
const readSensorValue = () => {  
    fetch("/sensor")
      ...
      .then(data => {
        ...
        const status = value >= 700 ? 'Toi' : 'Sang';
        document.getElementById("sensor_value").textContent = 
          `${value} (${status})`;
      });
  }
