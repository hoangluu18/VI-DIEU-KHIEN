...
#define GAS_SENSOR A0  // Chân analog cho cảm biến ánh sáng
...
void setup() {
  ...
  server.on("/gasValue", HTTP_GET, handleReadSensor);
...
void handleRoot() {
  ...
  html += "      document.getElementById('gasValue').innerHTML = this.responseText;";
  html += "    }";
  html += "  };";
  html += "  xhttp.open('GET', '/gasValue', true);";
  html += "  xhttp.send();";
  html += "}";
  ...
  html += "<body>";
  html += "<h1>Gas Sensor Value</h1>";
  html += "<p>Analog Value: <span id='gasValue'>" + String(gasValue) + "</span></p>";
