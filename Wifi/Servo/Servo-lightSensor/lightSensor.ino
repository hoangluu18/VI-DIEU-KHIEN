...
String generateHTML() {
  String html = "<!DOCTYPE html><html><head><title>ESP8266 LightSensor and Servo Control</title>";
  html += "<script>";
  ...
  html += "<p>Light state: <span id='light'>Loading...</span></p>";
...
void handleSensorRead() {
  ...
  String result = (val > 800) ? String("Toi (") + val + ")" 
                            : String("Sang (") + val + ")";
  //String result = String(val);
  Serial.println(result);

  // Trả về dữ liệu dưới dạng JSON
  String json = "{ \"light\": \"" + result + "\" }"; 
