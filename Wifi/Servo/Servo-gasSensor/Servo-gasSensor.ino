...
String generateHTML() {
  String html = "<!DOCTYPE html><html><head><title>ESP8266 GasSensor and Servo Control</title>";
  html += "<script>";
  ...
  html += "      document.getElementById('gas').innerHTML = data.gas;";
  html += "    }";
  html += "  };";
  ...
  html += "<p>Sensor result: <span id='gas'>Loading...</span></p>";
...
void handleSensorRead() {
  ...
  String result = (val > 700) ? String("co khi gas (") + val + ")" 
                            : String("khong co khi gas (") + val + ")";

  Serial.println(result); 

  // Trả về dữ liệu dưới dạng JSON
  String json = "{ \"gas\": \"" + result + "\" }"; 
